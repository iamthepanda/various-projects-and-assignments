...
--------------------------------------------------*/
#include <windows.h>//Our program needs this main windows header file to use
                    //Windows functions.
 
using namespace std;//Not to use std:: to use standard C++ library of classes etc.
 
LRESULT CALLBACK WndProc(HWND, UINT, WPARAM, LPARAM);//CALLBACK means that it will
                                                //be called by the operating system.
 
int WINAPI WinMain(HINSTANCE hCurrInstance,//A handle of the current instance of
                                            //our program.
    HINSTANCE hPreviousInstance,//A previous instance of our program if one exists.
    PSTR szCommandLine,//The pointer to null terminated string with command line
                        //without the program name.
    int nStartShow)//How to show our window at start: SW_HIDE, SW_MAXIMIZE,
                    //SW_SHOWMAXIMIZED, SW_SHOW etc
{MessageBeep(MB_OK);
    static char szOurProgramName[] = "Our C++ Hellow Programming World program";
                                                        //Program title to be
                                                        //displayed on the top
                                                        //of the window.
    HWND hMainWindow;//Letter h in the beginning of hMainWindow means that it's
                    //a handle to handle
                    //this window, the main window of our program or whatever.
    MSG WindowsMessageStruct;
 
    WNDCLASSEX MainWindowStruc;//This structure describes the main window of
                                //our program, its size etc.
                                //Then we fill this structure with our values
                                //or use default values.
    MainWindowStruc.cbSize = sizeof(MainWindowStruc);
    MainWindowStruc.style = CS_HREDRAW | CS_VREDRAW;
    MainWindowStruc.lpfnWndProc = WndProc;//The second main function of our program
                                            //to be called
                                            //when operating system is
                                            //sending a message to our program or its windows.
    MainWindowStruc.cbClsExtra = 0;//Bytes we may need to place after this structure.
                                    //In the name starting cb means count of bytes,
                                    //number of bytes.
    MainWindowStruc.cbWndExtra = 0;//Bytes to be used for creation of a dialog
                                    //after the program instance.
    ...
    MainWindowStruc.hIcon = LoadIcon(NULL, IDI_APPLICATION);//Default big Alt-Tab icon.
            //  MainWindowStruc.hIcon = LoadIcon(hCurrInstance, "Icon");
            //If we want to use our big Alt-Tab icon.
    MainWindowStruc.hCursor = LoadCursor(NULL, IDC_ARROW);//We use the default cursor here
                                                    //in order to not mess with resources.
    MainWindowStruc.hbrBackground =(HBRUSH) GetStockObject(WHITE_BRUSH);
                                                    //Background color of the
        //main window. Can be WHITE_BRUSH, BLACK_BRUSH, GRAY_BRUSH and several other values.
    MainWindowStruc.lpszMenuName = NULL;
                //This is a pointer to the name of our menue if we use one.
    MainWindowStruc.lpszClassName = szOurProgramName;//This string will be displayed
                                                //at the top of our main window.
...
//  MainWindowStruc.hIconSm = LoadIcon(hCurrInstance, "Icon");//This line will help us to
    ...
    RegisterClassEx(&MainWindowStruc);//After making WNDCLASSEX structure to create an actual
                                        //window we muct use this function.
...
LRESULT CALLBACK WndProc(HWND hOurWindowHandle,
                                //Again a handle to handle whatever we do with
                                                //our window.
    UINT iMessageToProcess, WPARAM wMessageParameterNr32bits,//Message parameter, may contain
                                                            //2 numbers, unsigned.
    LPARAM wMessageParameterLong)//Second message parameter, signed.
{
    HDC hDeviceContextHandle;
            //A handle to a device context structure which has all the information
            //needed in our window paiting operations.
    ...
    switch(iMessageToProcess)
    {
        ...
        case WM_PAINT://Operating system wants to repaint our program window.
            hDeviceContextHandle = BeginPaint(hOurWindowHandle, &OurPaintStructure);//Based on
...
//          FlashWindow(hOurWindowHandle, TRUE); // invert again
    ...
    return DefWindowProc(hOurWindowHandle,//We have to return this standard MS Windows function
                                            //to let Window to do whatever it usually does here.
        iMessageToProcess, wMessageParameterNr32bits, wMessageParameterLong);
}