...
class Decks {
			...
			for (int j = 0; j <= 3; j++) {
				for (int k = 1; k <= 13; k++) {
					try {
						...
						System.out.println("PlayingCardException: "
								+ e.getMessage());
					}
				}
			}
	...
	public static void main(String args[]) {
		...
		System.out
				.println("*******    Create 2 decks of cards      *********\n\n");
		...
		for (int j = 0; j < 2; j++) {
			...
			for (int i = 0; i < numHands; i++) {
				decks.shuffle();
				System.out
						.println("After shuffle:" + decks.remain() + " cards");
				System.out.println("\n\t" + decks);
				try {
					System.out.println("\n\nHand " + i + ":" + cardsPerHand
							+ " cards");
					...
					System.out
							.println("*** In catch block : PlayingCardException : msg : "
									+ e.getMessage());
				}
			}
